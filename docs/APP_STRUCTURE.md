...
- 시간, 스프링, 스태거, 이미지 전환 값은 `src/constants/motion.ts` 토큰만 사용한다.
- 터치 피드백은 짧고 작게, 화면 전환과 결과 모션은 의미가 구분될 만큼만 강조한다.
- 스와이프와 바텀시트 제스처는 UI thread에서 처리하고, 결과가 결정될 때만 JS로 전달한다.
- 반복 모션은 앱이 백그라운드에 있거나 사용자가 모션 줄이기를 켜면 즉시 멈춘다.
- 목록 스태거는 첫 5개까지만 적용하며 장식 애니메이션 때문에 콘텐츠 접근을 늦추지 않는다.
- 온라인 상태는 작은 presence ripple로 보조하되 색상과 텍스트 상태를 함께 제공한다.
- 햅틱은 선택, 결정, 결과 채널별 cooldown을 두어 빠른 연속 입력에서 겹치지 않게 한다.
- 개발 환경의 `/design-qa`에서 presence, skeleton, swipe, match, bottom sheet 모션을 함께 회귀 확인한다.
...
- 운영 센터는 큐 현황, 프로필 심사, 신고, 데이트 후 안전 신호, 감사 로그, 권한 관리로 분리한다.
- 긴급 신고와 24시간 초과 항목은 별도 우선순위와 필터로 표시하고 모든 종결 판단에는 운영 메모를 남길 수 있게 한다.
- 운영자는 일상 큐를 처리하고, 마스터만 권한 변경과 프로필 노출 중지를 실행한다.
