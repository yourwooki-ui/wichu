-- Discovery is presence-first: currently online profiles and then the most
-- recently active profiles are exposed before monetization or profile-fit ties.
...
as $$
  ...
  order by
    (candidate.last_active_at >= now() - interval '5 minutes') desc,
